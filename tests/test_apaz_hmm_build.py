...
class ApazHmmStructuralValidationTests(unittest.TestCase):
    """Every written HMM is reopened from disk and structurally proven."""
...
class ApazHmmBuildInputRejectionTests(unittest.TestCase):
    """The build refuses inputs that break the frozen APAZ reference contract."""

    def _write_reference_fixture_with_accession_moved_to(
        self, *, reference_directory: Path, accession: str, partition: str
    ) -> None:
        write_apaz_reference_fixture(reference_directory=reference_directory)
    ...
    def test_rejects_a_seed_accession_in_the_calibration_partition(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            reference_directory = Path(name) / "apaz_reference"
            self._write_reference_fixture_with_accession_moved_to(
                reference_directory=reference_directory,
                accession="APAZ1.1",
    ...
    def test_rejects_a_seed_accession_in_the_final_holdout_partition(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            reference_directory = Path(name) / "apaz_reference"
            self._write_reference_fixture_with_accession_moved_to(
                reference_directory=reference_directory,
