...
from src.pago_pipeline.clade_reference_catalog import (
    AGO_TYPE_TO_CLADE,
    KNOWN_ARCHITECTURE_CLADE_CONFLICTS,
...
class KnownArchitectureCladeConflictsTests(unittest.TestCase):
    """ADR-0011 requires AfAgo/SiAgo to stay UNRESOLVED and NgAgo to stay
    LONG_A; these are explicit scientific invariants, not incidental defaults."""

    def test_afago_is_quarantined_to_unresolved(self) -> None:
        result = apply_known_architecture_clade_conflicts(
            source_accession="WP_010878815.1",  # AfAgo
            source_phylogenetic_clade="LONG_B",
        )
        self.assertEqual(result["curated_pago_clade"], "UNRESOLVED")
        self.assertEqual(result["curation_status"], "QUARANTINE")
        self.assertIn("ADR-0011", result["quarantine_reason"])

    def test_siago_is_quarantined_to_unresolved(self) -> None:
        result = apply_known_architecture_clade_conflicts(
            source_accession="WP_012735993.1",  # SiAgo
            source_phylogenetic_clade="LONG_A",
        )
        self.assertEqual(result["curated_pago_clade"], "UNRESOLVED")
        self.assertEqual(result["curation_status"], "QUARANTINE")
        self.assertIn("ADR-0011", result["quarantine_reason"])

    def test_ngago_keeps_the_ryazansky_long_a_source_label(self) -> None:
        result = apply_known_architecture_clade_conflicts(
            source_accession="WP_005580376.1",  # NgAgo
            source_phylogenetic_clade="LONG_A",
        )
        self.assertEqual(result["curated_pago_clade"], "LONG_A")
        self.assertEqual(result["curation_status"], "OK")
        self.assertIn("ADR-0011", result["curation_notes"])

    def test_an_unlisted_accession_is_never_quarantined(self) -> None:
        result = apply_known_architecture_clade_conflicts(
            source_accession="WP_999999999.1",
            source_phylogenetic_clade="SHORT",
        )
        self.assertEqual(result["curated_pago_clade"], "SHORT")
        self.assertEqual(result["curation_status"], "OK")
        self.assertEqual(result["quarantine_reason"], "")

    def test_exactly_the_three_adr_0011_accessions_are_listed(self) -> None:
        self.assertEqual(
            set(KNOWN_ARCHITECTURE_CLADE_CONFLICTS),
            {"WP_010878815.1", "WP_012735993.1", "WP_005580376.1"},
        )
