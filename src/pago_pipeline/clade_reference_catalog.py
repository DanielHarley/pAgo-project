"""Pure parsing/coordinate logic for the Ryazansky 2018 pAgo catalog.
...
Nothing here clusters, aligns, partitions, or builds an HMM. Historical
roadmap reference: B4.2 (see ADR-0011).
"""
...
# Accessions whose independently curated architecture/literature clade
# evidence conflicts with the Ryazansky Table S1 phylogenetic clade, per
# ADR-0011 ("quarantine references whose architecture and source clade
# genuinely conflict instead of forcing a convenient label").
#
# This is a minimal, explicit substitute for the historical query-recall
# reference panel cross-check (`tests/fixtures/query_recall_reference_set.csv`,
# not part of this reintegration unit): only the three accessions ADR-0011
# documents by name are handled here, sourced from the frozen historical
# catalog produced by commit 481d0c2. It is not a general architecture- or
# literature-based cross-check against the full panel.
KNOWN_ARCHITECTURE_CLADE_CONFLICTS: Mapping[str, Mapping[str, str]] = {
    "WP_010878815.1": {  # AfAgo
        "curation_status": "QUARANTINE",
        "quarantine_reason": (
            "ADR-0011: architecture-based clade SHORT conflicts with the "
            "Ryazansky phylogenetic clade longB_trun; pending a specific "
            "audit against the primary AfAgo sources."
        ),
    },
    "WP_012735993.1": {  # SiAgo
        ...
        "quarantine_reason": (
            "ADR-0011: architecture-based clade SHORT conflicts with the "
            "Ryazansky phylogenetic clade longA_trun; pending a specific "
            "audit against the primary SiAgo sources."
        ),
    },
    "WP_005580376.1": {  # NgAgo
        "curation_status": "OK",
        "curation_notes": (
            "ADR-0011: an older query-recall-panel label recorded LONG_B for "
            "this accession while citing Ryazansky, but Table S1 records "
            "longA; treated as a probable recall-panel curation error. The "
            "Ryazansky classification (LONG_A) is retained."
        ),
    },
}


def apply_known_architecture_clade_conflicts(
    *, source_accession: str, source_phylogenetic_clade: str
) -> dict:
    """Apply the ADR-0011 architecture/clade conflict exceptions, if any.

    Returns ``curated_pago_clade``, ``curation_status``, ``quarantine_reason``,
    and ``curation_notes`` for ``source_accession``. An accession outside
    ``KNOWN_ARCHITECTURE_CLADE_CONFLICTS`` keeps its source clade unchanged
    and is never quarantined by this function.
    """
    conflict = KNOWN_ARCHITECTURE_CLADE_CONFLICTS.get(source_accession)
    if conflict is None:
        return {
            "curated_pago_clade": source_phylogenetic_clade,
            "curation_status": "OK",
            "quarantine_reason": "",
            "curation_notes": "",
        }
    curated_clade = (
        "UNRESOLVED" if conflict["curation_status"] == "QUARANTINE"
        else source_phylogenetic_clade
    )
    return {
        "curated_pago_clade": curated_clade,
        "curation_status": conflict["curation_status"],
        "quarantine_reason": conflict.get("quarantine_reason", ""),
        "curation_notes": conflict.get("curation_notes", ""),
    }
