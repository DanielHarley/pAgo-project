"""Derive the MID-PIWI high-similarity split groups for the clade reference layer.

Same workflow historically used for APAZ (ADR-0009), applied to the 1,002
MID-PIWI regions extracted by the Ryazansky catalog (ADR-0011):

  * MMseqs2 all-vs-all with ``--prefilter-mode 2`` (nofilter), full alignment,
    exporting ``qaln`` / ``taln`` (never ``pident`` / ``alnlen``);
  * identity and coverage recomputed by the reducer in
    ``src.pago_pipeline.clade_midpiwi_split_groups`` (see that module's
    docstring for why this PR does not import the historical
    ``apaz_split_groups.py`` reducer instead);
...
0.80) are reported but do NOT define split groups or leakage (ADR-0013).
Cross-clade at 0.70 / 0.50 is not treated as a conflict and never changes a
``curated_pago_clade`` or ``curation_status`` label.

No partitioning. No BUILD / CALIBRATION / FINAL_HOLDOUT. No TREE_BUILD /
PLACEMENT_CALIBRATION / PLACEMENT_HOLDOUT. No HMM. No tree.
...
Outputs : resources/clade_seed/split_groups/midpiwi/{split_groups.csv,
          groups.csv, pairs.filtered.tsv, diagnostic_thresholds.json,
          manifest.json, mmseqs_search.log}
"""
...
MMSEQS2_VERSION = {"version": "18.8cc5c", "build": "hd6d6fdc_0", "channel": "bioconda"}
...
def _diagnostic_pass(
    ...
    *, identity_threshold: float, coverage_threshold: float,
    ...
    return {
        ...
        # diagnostic evidence only - no quarantine, no exclusion, no relabeling
...
def derive(
    *, clade_seed_directory: Path, wsl_distro: str, reuse_pairs: Path | None = None,
    ...
    (output_directory / "diagnostic_thresholds.json").write_bytes(
        (json.dumps({
            "note": "diagnostic only; ONLY 0.90/0.80 defines split groups and "
                    "leakage; cross-clade at 0.70/0.50 is not a conflict and "
                    "never changes curated_pago_clade or curation_status",
    ...
    manifest = {
        ...
        "reducer_module": "src/pago_pipeline/clade_midpiwi_split_groups.py",
        "reducer_module_sha256": _sha256_file(
            PROJECT_ROOT / "src/pago_pipeline/clade_midpiwi_split_groups.py"
        ),
