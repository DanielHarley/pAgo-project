"""Build the deterministic Ryazansky 2018 pAgo catalog.
...
Responsibility boundary:
  ...
  * Parses Table S1 into ``ryazansky_s1_catalog.csv`` (1010 rows, one per
    ...
    Ryazansky itself declared unclassifiable, plus the two ADR-0011 exceptions
    (see below).
...
``clade_partitions.csv`` and never touches the APAZ or APAZ-HMM resources.
...
Adaptation note (ADR-0011 reintegration): the historical implementation of
this script cross-referenced every catalog row against the Phase A
query-recall reference panel (``tests/fixtures/query_recall_reference_set.csv``)
to flag rows whose independently curated clade disagreed with Ryazansky's.
That panel is a separate, not-yet-reintegrated unit and is intentionally not
depended on here. Instead, ``clade_reference_catalog.apply_known_architecture_
clade_conflicts`` applies only the three specific, ADR-0011-documented
exceptions (AfAgo, SiAgo quarantined to UNRESOLVED; NgAgo's LONG_A retained
with a note). This reproduces the same curated_pago_clade/curation_status
outcome ADR-0011 documents without a general, out-of-scope cross-check.
"""
...
from src.pago_pipeline.clade_reference_catalog import (  # noqa: E402
    apply_known_architecture_clade_conflicts,
...
# Files materialized as reference sources, with the role each plays.
...
def _sha256_text(text: str) -> str:
    return _sha256_bytes(text.encode("ascii"))
...
def fetch_sources(*, source_data_directory: Path, session: requests.Session) -> dict:
    ...
    # If the APAZ reference layer has already frozen this same article's PMC
    # metadata, cross-check byte identity; that resource is not a prerequisite
    # for this script, so the check is skipped when it is absent.
    frozen_apaz_metadata = (
        PROJECT_ROOT / "src" / "pago_pipeline" / "resources" / "apaz_seed"
        / "source_data" / f"{PMC_ID}.metadata.json"
    )
    if frozen_apaz_metadata.is_file():
        if _sha256_bytes(metadata_payload) != _sha256_bytes(
            frozen_apaz_metadata.read_bytes()
        ):
            raise RuntimeError(
                "PMC metadata for PMC6299218.1 no longer matches the copy frozen "
                "in the APAZ reference resources; provenance chain broken."
            )
    declared_md5 = {}
...
def build_catalog(
    *,
    ...
    for record in records:
        ...
        # Ryazansky itself declared two proteins unclassifiable (high
        # divergence); never a LONG_A/LONG_B/SHORT seed.
        if record["source_ago_type"] == "unkn":
            curation = {
                "curated_pago_clade": "UNRESOLVED",
                "curation_status": "OK",
                "quarantine_reason": "",
                "curation_notes": (
                    "Ryazansky declared this protein unclassifiable (high "
                    "divergence); curated_pago_clade=UNRESOLVED; never a "
                    "LONG_A/LONG_B/SHORT seed."
                ),
            }
        else:
            curation = apply_known_architecture_clade_conflicts(
        ...
        catalog.append({
            ...
            "ago_family": "PAGO",
            "source_phylogenetic_clade": record["source_phylogenetic_clade"],
            "curated_pago_clade": curation["curated_pago_clade"],
            "architecture_status": (
            ...
            "quarantine_reason": curation["quarantine_reason"],
            "curation_notes": curation["curation_notes"],
        })
...
CATALOG_FIELDNAMES = [
    ...
    "midpiwi_extraction_status", "curation_status", "quarantine_reason",
    "curation_notes",
]
